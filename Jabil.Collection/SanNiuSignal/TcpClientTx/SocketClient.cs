...
namespace SanNiuSignal
{
    ...
    public class SocketClient : TcpFTxBase, ITxClient
    {
        ...
        private bool outtimebool; //超时用到的临时变量
        private int reconnectCi; //已经重连了几次的临时变量
        private bool reconnectOn; //有没有在重连的临时变量
        public TcpState stateOne;
        ...
        private void start()
        {
            ...
            try
            {
                ...
                socket.BeginConnect(IpEndPoint, AcceptCallback, socket);
        ...
        private void AcceptCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                socket.BeginReceive(stateOne.Buffer, 0, stateOne.Buffer.Length, 0, ReadCallback,
                    stateOne);
            }
            catch (Exception Ex)
            {
        ...
        private void heartThread()
        {
            while (true)
            {
                ...
                if ((int) (DateTime.Now - stateOne.HeartTime).TotalSeconds > HeartTime*4) //4次没有收到失去联系
                {
