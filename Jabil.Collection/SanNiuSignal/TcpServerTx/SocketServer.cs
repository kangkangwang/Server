...
namespace SanNiuSignal
{
    ...
    public class SocketServer : TcpFTxBase, ITxServer
    {
        ...
        private int _clientMax = 100; //允许最多客户端数
        private Socket listener;
        private List<TcpState> state; //所有客户端

        /// <summary>
        ///     带参数的构造函数
        /// </summary>
        /// <param name="port">端口号</param>
        internal SocketServer(string ip, int port)
        {
            Ip = ip;
            Port = port;
            if (state == null)
        ...
        List<IPEndPoint> ITxServer.ClientAll
        {
            get
            {
                ...
                var IpEndPoint = new List<IPEndPoint>();
        ...
        private void AcceptCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                var threadLongin = new Thread(loginInitialization);
        ...
        {
            var stateOne = (TcpState) ar.AsyncState;
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    handler.BeginReceive(stateOne.Buffer, 0, stateOne.Buffer.Length, 0, ReadCallback,
                        stateOne);
        ...
        internal override void Send(StateBase stateBase, byte[] data)
        {
            ...
            try
            {
                stateBase.WorkSocket.BeginSend(data, 0, data.Length, 0, SendCallback, stateBase);
            }
            catch (Exception Ex)
            {
                int i = Ex.Message.IndexOf("远程主机强迫关闭了一个现有的连接");
        ...
        {
            var stateBase = (StateBase) ar.AsyncState;
            if (stateBase == null)
            {
                return;
            }
        ...
        private void heartThread()
        {
            while (true)
            {
                ...
                while (i < state.Count)
                {
                    ...
                    if ((int) (DateTime.Now - state[i].HeartTime).TotalSeconds > HeartTime*4) //4次没有收到失去联系
                    {
        ...
        private void socketRemove(TcpState stateOne, string str)
        {
            ...
            if (state.Remove(stateOne)) //当没有登录的时候断掉，不触发下面的事件
            {
                ...
                FileOperate(stateOne.IpEndPoint + "已经断开"); //记录
        ...
        {
            var stateOne = (TcpState) stateOne1;
            ...
            Send(stateOne, EncryptionDecryptVerification.EncryptionVerification(PasswordCode._serverToClient));
            //发送登录成功的代码
            try
            {
                stateOne.WorkSocket.BeginReceive(stateOne.Buffer, 0, stateOne.Buffer.Length, 0,
                    ReadCallback, stateOne);
            }
            catch
            {
            }
            ...
            {
                Thread.Sleep(100);
                if (stateOne == null || stateOne.ConnectOk)
                {
                    break;
                }
                if ((int) (DateTime.Now - dateOne).TotalSeconds > 2)
                {
                    ...
                    FileOperate(stateOne.IpEndPoint + "无法收到客户端登录信息"); //记录;
                    break;
                }
            }
        }
        ...
        private void loginSuccess(TcpState stateOne)
        {
            ...
            FileOperate(stateOne.IpEndPoint + "登陆成功"); //记录
        }
        ...
        ///     关闭服务器,释放所有资源
        /// </summary>
        public override void CloseEngine()
        {
            try
            {
                if (HeartThread != null)
        ...
        private void clientClose(TcpState stateOne)
        {
            ...
            Send(stateOne, EncryptionDecryptVerification.EncryptionVerification(PasswordCode._clientCloseCode));
            //发送一个强制关闭的代码过去
        }
