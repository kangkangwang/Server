...
namespace SanNiuSignal.PasswordManage
{
    ...
    internal class EncryptionDecryptSeparateDate
    {
        ...
        internal static byte[] SendHeadEncryption(byte[] date, byte textCode, StateBase state)
        {
            ...
            var headDate = new byte[11];
            headDate[0] = PasswordCode._bigDateCode;
            headDate[1] = PasswordCode._fileHeadCode;
            headDate[2] = textCode;
...
//收到的是文件主体部分
                ...
                byte[] ReplyDate = ByteToDate.CombinationTwo(PasswordCode._bigDateCode, PasswordCode._dateSuccess,
                    state.ReceiveFile.FileLabel);
                ...
                {
                    var FileDateAll = new byte[state.ReceiveFile.FileDateAll.Length + dateAll.Length];
        ...
        private static byte[] FileGetSendDate(StateBase state)
        {
            ...
            {
                var dateS = new byte[Date_Max];
                var dateL = new byte[FileDateAllLenth - Date_Max];
