    ...
    public class ReceiveFile : FileToBase, IFileReceive
    {
        ...
        internal byte[] ReceiveDateTO(byte[] receiveToDate, StateBase stateOne)
        {
            ...
            if (code == PasswordCode._fileHeadCode) //说明发送过来的是文件是否传输的包头信息
            {
                ...
                if (fileName == "")
                {
                    haveDate = EncryptionDecryptFile.FileSevenEncryption(PasswordCode._receiveUser,
                        PasswordCode._fileRefuse, fileLabel);
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        stateHead.FileName = fileName;
                        stateHead.Filestream = fs;
                        stateHead.StateFile = 1;
                        FS.Add(stateHead);
                        haveDate = EncryptionDecryptFile.FileSevenEncryption(PasswordCode._receiveUser,
                            PasswordCode._fileOk, fileLabel);
                    }
                }
                return haveDate;
            }
            ...
            if (state == null)
            {
                ...
                return EncryptionDecryptFile.FileSevenEncryption(PasswordCode._receiveUser, PasswordCode._fileCancel,
                    fileLabel);
            }
            switch (code)
            {
                case PasswordCode._fileSubjectCode: //收到的是文件主体代码
                    ...
                    if (state.FileOkLenth >= state.FileLenth) //说明文件接收完成了
                    {
                        ...
                        ReceiveMust.ReceiveSuccess(fileLabel);
                    }
                    break;
                ...
                case PasswordCode._fileContinue: //对方发过来一个续传的确认信息;你是否同意；
                    ...
                    if (orStop)
                    {
                        haveDate = EncryptionDecryptFile.ReceiveContingueEncryption(PasswordCode._fileContinueOk,
                            state);
                        state.StateFile = 1;
                    ...
                    {
                        haveDate = EncryptionDecryptFile.FileSevenEncryption(PasswordCode._receiveUser,
                            PasswordCode._fileContinueNo, fileLabel);
                    }
                    break;
