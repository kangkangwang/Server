    ...
    public class FileToBase
    {
        ...
        public void FileCancel(int FileLabel)
        {
            try
            {
                ...
                try
                {
                    int offset1 = FS.FindIndex(delegate(FileState state1) { return state1.StateOne == state2.StateOne; });
                }
                catch
                {
                    if (state2.StateOne.Buffer.Length != state2.StateOne.BufferSize)
                    {
                        state2.StateOne.BufferBackup = state2.StateOne.Buffer;
                        state2.StateOne.Buffer = new byte[state2.StateOne.BufferSize];
                    }
                }
            }
            catch
        ...
        public void FileCancelAll()
        {
            FS.ForEach(delegate(FileState state)
            {
                ...
                if (state.StateOne.Buffer.Length != state.StateOne.BufferSize)
                {
                    state.StateOne.BufferBackup = state.StateOne.Buffer;
                    state.StateOne.Buffer = new byte[state.StateOne.BufferSize];
                }
            });
