...
namespace SanNiuSignal.FileCenter.FileSend
{
    ...
    public class SendFile : FileToBase, IFileSend
    {
        ...
        internal byte[] FileContinue(int fileLable, StateBase stateOne)
        {
            ...
            {
                haveDate = EncryptionDecryptFile.FileSevenEncryption(PasswordCode._sendUser, PasswordCode._fileContinue,
                    fileLable);
        ...
        internal byte[] ReceiveDateTO(byte[] receiveToDate, StateBase stateOne)
        {
            ...
            if (state == null)
            {
                ...
                return EncryptionDecryptFile.FileSevenEncryption(PasswordCode._sendUser, PasswordCode._fileCancel,
                    fileLabel);
            }
            switch (code)
            {
                ...
                case PasswordCode._dateSuccess: //主体部分数据发送成功；准备发送下一批
                    ...
                    {
                        FileRemove(fileLabel);
                        SendMust.SendSuccess(fileLabel);
                    }
                    break;
                ...
                case PasswordCode._fileContinue: //对方发过来一个续传的确认信息;你是否同意；
                    ...
                    {
                        haveDate = EncryptionDecryptFile.FileSevenEncryption(PasswordCode._sendUser,
