...
namespace Jabil.Collection
{
    ...
    internal class Program
    {
        ...
        public static void Main()
        {
            ...
            if (isCollect == 1)
            {
                var threadCollect = new Thread(CreateCollect);
                threadCollect.Start();
            }
            if (isSocket == 1)
            {
                var threadSocket = new Thread(CreateSocket);
                threadSocket.Start();
            }
            if (isAlarmSys == 1)
            {
                var threadAlarm = new Thread(CreateAlarmSms);
                threadAlarm.Start();
            }
            if (isListenAdobe == 1)
            {
                var threadListenAdobe = new Thread(CreateListenAdobe);
                threadListenAdobe.Start();
            }
            if (isListenExcep == 1)
            {
                var threadListenExcep = new Thread(CreateListenExcep);
        ...
        ///     创建采集程序
        /// </summary>
        private static void CreateCollect()
        {
            collectCenter = new CollectCenter_Jabil();
            collectCenter.Start();
        }
        ...
        ///     创建短信报警
        /// </summary>
        private static void CreateAlarmSms()
        {
            smsAlarmCenter = new SmsAlarmCenter();
            smsAlarmCenter.Start();
        }
        ...
        ///     创建Adobe监听
        /// </summary>
        private static void CreateListenAdobe()
        {
            listenAdobeCenter = new ListenAdobeCenter();
            listenAdobeCenter.Start();
        }
        ...
        ///     创建Smtp监听
        /// </summary>
        private static void CreateListenExcep()
        {
            exceptionCenter = new ExceptionCenter();
            exceptionCenter.Start();
        }
